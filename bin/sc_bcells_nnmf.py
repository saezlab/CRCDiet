...
parser.add_argument('-ct', '--cell_type', help="Cell type of interest", required=True)
...
def analyse_nmf_results(random_state, n_of_factors=20):

    processed_sample_dict = dict()
    adata_bcells_merged = utils.read_pickle(os.path.join(OUT_DATA_PATH, f'adata_bcells_merged_nnmf_{random_seed}.pckl'))
    ...
    for ind, row in meta.iterrows():
        ...
        adata_bcells_cond = adata_bcells_merged[adata_bcells_merged.obs["condition"]==condition,:].copy()        
    ...
    for factor_ind in range(1,(n_of_factors+1)):
        ...
        for _, row in meta.iterrows():
            ...
            if "Immune" in condition:
                ...
                sc.pl.umap(processed_sample_dict[sample_id], size=50, title=condition, color=f"W{n_of_factors}_{factor_ind}", ax = axsLeft[fig_row], show=False)
        ...
        top40_genes = adata_bcells_merged.var[f"H{n_of_factors}_{factor_ind}"].sort_values(ascending=False)[:40]
        # print(top40_genes)
...
adata_raw = utils.get_filtered_concat_data(sample_type)

adata_raw = adata_raw[adata_integ_clust.obs_names,:]
sc.pp.normalize_total(adata_raw, target_sum=1e6)
sc.pp.log1p(adata_raw)

adata_raw.obs["leiden_0.20"] = adata_integ_clust.obs["leiden_0.20"]
adata_raw.obsm["X_umap"] = adata_integ_clust.obsm["X_umap"]

print("Performing NMF... Number of factors: 3")
# W = num of cells x factor
# H = factor x num of genes
W3, H3 = run_NMF(adata_raw.X, 3, random_state=random_seed)

for factor_ind in range(W3.shape[1]):
    adata_raw.obs[f"W3_{factor_ind+1}"] = W3[: , factor_ind]

for factor_ind in range(H3.shape[0]):
    adata_raw.var[f"H3_{factor_ind+1}"] = H3[factor_ind , :]


print("Performing NMF... Number of factors: 5")
# W = num of cells x factor
# H = factor x num of genes
W5, H5 = run_NMF(adata_raw.X, 5, random_state=random_seed)

for factor_ind in range(W5.shape[1]):
    adata_raw.obs[f"W5_{factor_ind+1}"] = W5[: , factor_ind]

for factor_ind in range(H5.shape[0]):
    adata_raw.var[f"H5_{factor_ind+1}"] = H5[factor_ind , :]

print("Performing NMF... Number of factors: 20")
W20, H20 = run_NMF(adata_raw.X, 20, random_state=random_seed)

for factor_ind in range(W20.shape[1]):
    adata_raw.obs[f"W20_{factor_ind+1}"] = W20[: , factor_ind]

for factor_ind in range(H20.shape[0]):
    adata_raw.var[f"H20_{factor_ind+1}"] = H20[factor_ind , :]
    
utils.write_pickle(os.path.join(OUT_DATA_PATH,f'adata_bcells_merged_nnmf_{random_seed}.pckl'), adata_raw)


analyse_nmf_results(42, 20)
analyse_nmf_results(42, 3)
analyse_nmf_results(42, 5)




# python sc_bcells_nnmf.py -i ../data/out_data/sc_integrated_clustered.h5ad -o ../data/out_data -an sc_bcells_nnmf -ct "B cells"